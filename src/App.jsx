  ...
  <img src="/barril.png" alt="barril" style={{ width: 90, height: 90,
    ...
    filter: "drop-shadow(0 4px 16px rgba(0,0,0,0.6))" }} />
      ...
      <div style={{ zIndex: 1, display: "flex", flexDirection: "column",
        ...
        <img src="/logo.png" alt="Logo" style={{ height: 110, objectFit: "contain",
          borderRadius: 14, padding: 10, background: "rgba(255,255,255,0.04)",
          border: "1px solid #f5a62220" }} />
  ...
  return (
    <div style={{ background: "#0a0500", minHeight: "100vh" }}>
      ...
      <div style={{ background: "linear-gradient(90deg, #1a0a00 0%, #0f0800 50%, #1a0a00 100%)",
        borderBottom: "1px solid #f5a62325", padding: "0.6rem 1.5rem",
        display: "flex", justifyContent: "space-between", alignItems: "center",
        position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <img src="/logo.png" alt="Logo" style={{ height: 34, objectFit: "contain", borderRadius: 6 }} />
          <span style={{ color: "#f5a623", fontWeight: 800, fontSize: "1rem" }}>
            Mundial Imporbeer 2026
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <img src={usuario.photoURL} alt="foto" style={{ width: 32, height: 32,
            borderRadius: "50%", border: "2px solid #f5a62350" }} />
          <span style={{ color: "#888", fontSize: 13 }}>{usuario.displayName}</span>
      ...
      <div style={{ background: "linear-gradient(90deg, #1a0a00, #0f0800, #1a0a00)",
        ...
        gap: 6, flexWrap: "nowrap", overflow: "hidden" }}>
        {todosLosEquipos.map(e => (
          <Bandera key={e.code} code={e.code} size={14} />
        ))}
      </div>

      {/* Pestañas con balón imagen */}
      <div style={{ display: "flex", borderBottom: "1px solid #1a1000",
        background: "#0a0500", padding: "0 1rem" }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setPestana(tab.id)} style={{
            ...
            {tab.id === "pronosticos" && (
              <img src="/balon.png" alt="" style={{ width: 16, height: 16, objectFit: "contain" }} />
            )}
            {tab.id === "historial" && "📋 "}
            {tab.id === "tabla" && "🏆 "}
            {tab.label}
      ...
      {/* Contenido con decoración lateral simétrica */}
